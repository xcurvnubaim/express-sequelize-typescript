...
export const globalErrorHandler = (
  ...
  if (error.name === 'ValidationError' || error.name === 'ZodError') {
    const apiError = new ApiErrorClass('Validation failed', 400, error);
  ...
  if (error.name === 'JsonWebTokenError' || error.name === 'TokenExpiredError') {
    const apiError = new ApiErrorClass('Invalid or expired token', 401, error);
  ...
  if (error.name === 'SequelizeValidationError' || error.name === 'SequelizeUniqueConstraintError') {
    const apiError = new ApiErrorClass('Database validation failed', 400, error);
  ...
  if (error.name === 'SequelizeForeignKeyConstraintError') {
    const apiError = new ApiErrorClass('Foreign key constraint violation', 400, error);
  ...
  if (error.name === 'SequelizeDatabaseError') {
    const apiError = new ApiErrorClass('Database operation failed', 500, error);
  ...
  if (error.message?.toLowerCase().includes('not found')) {
    const apiError = new ApiErrorClass(error.message, 404, error);
  ...
  const apiError = new ApiErrorClass(message, 500, error);
